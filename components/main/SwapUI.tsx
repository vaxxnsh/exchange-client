...
export function SwapUI({ market }: {market: string}) {
    ...
    const handlePostOnlyChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const handlePercentageClick = (percentage: number) => {
        ...
        const availableBalance = 36.94; // USDC
        const priceValue = parseFloat(price) || 0;
        
        if (priceValue > 0) {
            const maxQuantity = availableBalance / priceValue;
                            ...
                            <div className="flex flex-col relative">
                                <input 
                                    ...
                                    className="h-12 rounded-lg border-2 border-solid 
                                                border-baseBorderLight bg-overfore pr-12 
                                                text-right text-2xl leading-9 text-[$text] 
                                                placeholder-baseTextMedEmphasis ring-0 transition 
                                                focus:border-accentBlue focus:ring-0" 
                                    type="text" 
                                    value={price}
                    ...
                    <div className="flex flex-col gap-2">
                        ...
                        <div className="flex justify-end flex-row">
                            <p className="font-medium pr-2 text-xs text-baseTextMedEmphasis">
                                ≈ {(parseFloat(quantity) * parseFloat(price) || 0).toFixed(2)} USDC
                            </p>
                        </div>
